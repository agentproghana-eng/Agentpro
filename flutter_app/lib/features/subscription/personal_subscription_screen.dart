...
class _PersonalSubscriptionScreenState extends State<PersonalSubscriptionScreen>
  ...
  Future<bool> _confirmPaystackPayment() async {
    ...
    final amount = rawAmount is num
        ? rawAmount.toDouble()
        : (double.tryParse(rawAmount?.toString() ?? '') ?? 5);
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text(
              'AgentPro Personal Subscription',
              style: TextStyle(
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (rawData is! Map) {
        throw const FormatException('Invalid Personal subscription response.');
      }
      ...
      if (rawSubscription is! Map) {
        throw const FormatException('Invalid Personal subscription status.');
      }
