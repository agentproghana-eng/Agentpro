...
final List<HelpSection> _helpSections = [
  const HelpSection(
      ...
      items: [
        ...
        HelpItem(
          ...
          answer:
              'New staff receive a secure one-time password setup link by email. No password is sent by email, SMS, or push notification. The setup link expires after one hour. If it expires or does not arrive, the staff member can use Forgot Password in the app.',
        ),
      ]),
  const HelpSection(
  ...
  const HelpSection(title: 'Transaction History', icon: Icons.history, items: [
    ...
    HelpItem(
      ...
      answer:
          'Tap the sort icon at the top of the screen — choose Date, Amount, Commission, or Agent Service Fees, ascending or descending.',
    ),
  ]),
  const HelpSection(
      title: 'Reports',
