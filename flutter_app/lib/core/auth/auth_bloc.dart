...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onRegisterPersonal(AuthRegisterPersonalEvent event, Emitter<AuthState> emit) async {
    emit(AuthLoading());
    try {
      final response = await ApiClient.instance.post('/auth/register-personal', data: {
      ...
      if (e.toString().contains('409')) message = 'An account with this email already exists.';
      emit(AuthError(message));
    }
  }
