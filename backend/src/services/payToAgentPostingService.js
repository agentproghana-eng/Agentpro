...
// and new physical-SIM activity must never steal or silently attribute
// that balance.
async function postPayToAgent(
  client,
  transaction,
  agentId
) {
  if (!transaction) {
    throw new Error(
      "Transaction is required for Pay to Agent posting"
    );
  }
  ...
  if (!agentId) {
    throw new Error(
      "agentId is required for Pay to Agent posting"
    );
  }
  ...
  if (amount <= 0) {
    throw new Error(
      "Pay to Agent amount must be positive"
    );
  }
  ...
  if (transactionWalletLink.rows.length !== 1) {
    throw new Error(
      "Unable to link Pay to Agent transaction to SIM wallet"
    );
  }
  ...
  if (walletUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Pay to Agent SIM wallet"
    );
  }
  ...
  if (cashUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Pay to Agent cash drawer"
    );
  }
  ...
  // Physical side: cash received from the other agent enters the
  // initiating agent's one cash drawer.
