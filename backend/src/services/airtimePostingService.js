...
// Posts the agent-side accounting effect of a confirmed customer Airtime.
//
...
async function postAirtime(
  ...
  if (!transaction) {
    throw new Error(
      "Transaction is required for Airtime posting"
    );
  }

  if (transaction.transaction_type !== "airtime") {
    throw new Error(
      "postAirtime can only post airtime transactions"
    );
  }

  if (!agentId) {
    throw new Error(
      "agentId is required for Airtime posting"
    );
  }
  ...
  if (!supportedProviders.has(transaction.provider)) {
    throw new Error(
      "Airtime posting is only supported for MTN, Telecel, or AT Money"
    );
  }
  ...
  if (amount <= 0) {
    throw new Error(
      "Airtime amount must be positive"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Airtime transaction SIM wallet does not match its stored SIM identity"
    );
  }
  ...
  if (transactionWalletLink.rows.length !== 1) {
    throw new Error(
      "Unable to link Airtime transaction to SIM wallet"
    );
  }
  ...
  if (walletUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Airtime SIM wallet"
    );
  }
  ...
  if (cashUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Airtime cash drawer"
    );
  }
