...
// Posts the agent-side accounting effect of a confirmed customer Data Bundle.
//
...
async function postDataBundle(
  ...
  if (!transaction) {
    throw new Error(
      "Transaction is required for Data Bundle posting"
    );
  }

  if (transaction.transaction_type !== "data_bundle") {
    throw new Error(
      "postDataBundle can only post data_bundle transactions"
    );
  }

  if (!agentId) {
    throw new Error(
      "agentId is required for Data Bundle posting"
    );
  }
  ...
  if (amount <= 0) {
    throw new Error(
      "Data Bundle amount must be positive"
    );
  }
  ...
  if (transactionWalletLink.rows.length !== 1) {
    throw new Error(
      "Unable to link Data Bundle transaction to SIM wallet"
    );
  }
  ...
  if (walletUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Data Bundle SIM wallet"
    );
  }
  ...
  if (cashUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Data Bundle cash drawer"
    );
  }
