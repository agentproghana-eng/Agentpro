...
const normalizePurpose = (value) => {
  ...
  return normalized === "personal" ? "subscriber" : normalized;
};
...
const failure = (code, message, status = 409) => ({
  ok: false,
  status,
  code,
  message,
...
async function verifyBusinessSimRoleAssignment({
  ...
  const normalizedRole = normalizePurpose(claimedRole);
  ...
  const normalizedSubscriptionId = normalizeInteger(simSubscriptionId);

  if (BUSINESS_SIM_ROLES.includes(normalizedRole) === false) {
    return failure(
  ...
  const hasSlot = Number.isInteger(normalizedSlot) && normalizedSlot >= 0;

  const hasIdentifiedIdentity = hasSlot && normalizedIccid.length > 0;
  ...
  if (hasIdentifiedIdentity === false && hasFallbackIdentity === false) {
    return failure(
  ...
  const assignmentResult = await queryFn(
    `SELECT
         sim_slot,
         sim_iccid,
  ...
  const storedProvider = normalizeLower(assignment.provider);

  if (sameText(storedProvider, normalizedProvider) === false) {
    return failure(
  ...
  const storedRole = normalizePurpose(assignment.purpose);

  if (sameText(storedRole, normalizedRole) === false) {
    return failure(
      "SIM_ROLE_MISMATCH",
  ...
  const storedIccid = normalizeText(assignment.sim_iccid);

  const storedInstallationId = normalizeText(assignment.installation_id);

  const storedSubscriptionId = normalizeInteger(assignment.sim_subscription_id);

  const fallbackMatches =
    normalizedInstallationId.length > 0 &&
    storedInstallationId.length > 0 &&
    sameText(storedInstallationId, normalizedInstallationId) &&
    Number.isInteger(normalizedSubscriptionId) &&
    sameText(storedSubscriptionId, normalizedSubscriptionId);

  if (normalizedIccid.length > 0 && storedIccid.length > 0) {
    if (sameText(storedIccid, normalizedIccid) === false) {
      return failure(
        "SIM_ROLE_IDENTITY_MISMATCH",
        "The physical SIM does not match the SIM that owns the saved role assignment.",
      );
    }
  } else if (fallbackMatches === false) {
    return failure(
