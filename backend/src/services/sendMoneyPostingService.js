...
// Posts the agent-side accounting effect of a confirmed customer Send Money.
//
...
async function postSendMoney(
  ...
  if (!transaction) {
    throw new Error(
      "Transaction is required for Send Money posting"
    );
  }

  if (transaction.transaction_type !== "send_money") {
    throw new Error(
      "postSendMoney can only post send_money transactions"
    );
  }

  if (!agentId) {
    throw new Error(
      "agentId is required for Send Money posting"
    );
  }

  const supportedProviders =
    new Set(["mtn", "telecel", "at_money"]);

  if (!supportedProviders.has(transaction.provider)) {
    throw new Error(
      "Send Money posting is only supported for MTN, Telecel, or AT Money"
    );
  }
  ...
  if (amount <= 0) {
    throw new Error(
      "Send Money amount must be positive"
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Send Money transaction SIM wallet does not match its stored SIM identity"
    );
  }
  ...
  if (transactionWalletLink.rows.length !== 1) {
    throw new Error(
      "Unable to link Send Money transaction to SIM wallet"
    );
  }
  ...
  if (walletUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Send Money SIM wallet"
    );
  }
  ...
  if (cashUpdate.rows.length !== 1) {
    throw new Error(
      "Unable to update Send Money cash drawer"
    );
  }
