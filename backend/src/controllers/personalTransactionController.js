...
exports.completeTransaction = async (req, res) => {
  const { status, network_reference, notes } = req.body;
  const { transaction_id } = req.params;
  const userId = req.user.id;

  try {
    const existing = await query(
      'SELECT id FROM personal_transactions WHERE id = $1 AND user_id = $2',
    ...
    const result = await query(
      ...
      [status, network_reference || null, notes || null, transaction_id]
    );

    await auditLog({
      userId,
      companyId: null,
      action: 'PERSONAL_TRANSACTION_COMPLETED',
      entityType: 'personal_transaction',
