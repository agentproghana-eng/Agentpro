...
jest.mock('../../src/utils/ussdFlowCapabilities', () => ({
  getRegisteredProviders: (...args) =>
    mockGetRegisteredProviders(...args),
}));
...
describe('Quick Action catalog controller behavior', () => {
  ...
  test(
    'Business catalog groups Global flow variants under one dynamic action',
    async () => {
  ...
  test(
    'Personal catalog passes personal account mode to the capability query',
    async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [
          {
  ...
  test(
    'agent mode is normalized to the Business capability catalog',
    async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [],
      });
  ...
  test(
    'invalid catalog mode is rejected before database work',
    async () => {
      const req = {
        user: makeUser(),
        query: {
  ...
  test(
    'Quick Action preferences accept a newly registered provider',
    async () => {
  ...
  test(
    'Quick Action preferences reject an unregistered provider before update',
    async () => {
      mockGetRegisteredProviders.mockResolvedValue([
        'mtn',
        'telecel',
      ...
      expect(res.json).toHaveBeenCalledWith({
        success: false,
        message:
          'Invalid provider in agent_quick_actions: invented_money',
      });
